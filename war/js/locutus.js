					...
					$(document)
							.on(
									...
									function(event) {
										...
										q.attr("class", "question"); // Remove editing class,
										// removeClass() didn't
																	...
																	.each(
																			function() {
																				...
																				if (response.answers[ln]) {
																					...
																					$(this).find("DIV.answer").html(
																							response.answers[ln]);
																					highlightSyntax($(this).find(
																							"DIV.question"));
																					if (!$(this).find("DIV.question").is(
																							":focus")) {
																						// Only show if this question
																						// doesn't currently have focus
																						var answerType = response.answerTypes[ln];
																						if (answerType == "NORMAL") {
																							$(this).find("DIV.answer")
																									.fadeIn("fast");
																							$(this).find("DIV.equals").text(
																									"=").fadeIn("fast");
																							...
																							$(this)
																									.find("DIV.equals")
																									.html(
																											"<span style=\"font-size:10pt;\">&#10003</span>")
																									.fadeIn("fast");
																						}
																					}
																				}
